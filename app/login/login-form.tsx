"use client";

import { useState, Suspense } from "react";
import { useRouter, useSearchParams } from "next/navigation";
import { signInWithPopup, GoogleAuthProvider } from "firebase/auth";
import { auth } from "@/lib/firebase/client";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { FlaskConical, Loader2 } from "lucide-react";
import SiteCreditFooter from "@/components/site-credit-footer";

export default function LoginForm({ loadTestEnabled }: { loadTestEnabled: boolean }) {
  return (
    <Suspense>
      <LoginFormInner loadTestEnabled={loadTestEnabled} />
    </Suspense>
  );
}

function LoginFormInner({ loadTestEnabled }: { loadTestEnabled: boolean }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const redirect = searchParams.get("redirect") || "/dashboard";

  const [loading, setLoading] = useState(false);

  async function handleGoogleSignIn() {
    setLoading(true);

    try {
      const provider = new GoogleAuthProvider();
      const credential = await signInWithPopup(auth, provider);
      const idToken = await credential.user.getIdToken();

      const res = await fetch("/api/auth/session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ idToken }),
      });

      if (!res.ok) throw new Error("Failed to create session");
      router.push(redirect);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Sign-in failed";
      if (msg.includes("popup-closed-by-user") || msg.includes("cancelled")) {
        setLoading(false);
        return;
      }
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  }

  async function handleTestSignIn() {
    setLoading(true);
    try {
      const res = await fetch("/api/auth/test-session", { method: "POST" });
      if (!res.ok) {
        const body = (await res.json().catch(() => null)) as { error?: string } | null;
        throw new Error(body?.error ?? "Test sign-in failed");
      }
      router.push(redirect);
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : "Test sign-in failed");
    } finally {
      setLoading(false);
    }
  }

  return (
    <div className="gradient-hero flex min-h-screen flex-col">
      <div className="flex flex-1 items-center justify-center px-4 py-10">
        <Card className="w-full max-w-sm border-white/20 bg-white/10 backdrop-blur-md text-white shadow-2xl">
          <CardHeader className="space-y-1 text-center">
            <div className="mb-2 flex justify-center">
              <div className="gradient-brand flex h-10 w-10 items-center justify-center rounded-full shadow-lg">
                <span className="text-lg font-bold text-white">C</span>
              </div>
            </div>
            <CardTitle className="text-2xl text-white">Cursor Community</CardTitle>
            <CardDescription className="text-white/70">
              Event Coupon Distribution Platform
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {loadTestEnabled ? (
              <Button
                variant="outline"
                className="w-full border-0 bg-white text-primary shadow-md hover:bg-white/90"
                onClick={handleTestSignIn}
                disabled={loading}
              >
                {loading ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" />
                    Signing in…
                  </>
                ) : (
                  <>
                    <FlaskConical className="h-4 w-4" />
                    Test sign in
                  </>
                )}
              </Button>
            ) : (
              <Button
                variant="outline"
                className="w-full border-0 bg-white text-primary shadow-md hover:bg-white/90"
                onClick={handleGoogleSignIn}
                disabled={loading}
              >
                {loading ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" />
                    Signing in…
                  </>
                ) : (
                  <>
                    <GoogleIcon />
                    Sign in with Google
                  </>
                )}
              </Button>
            )}

            <p className="text-center text-xs text-white/50">
              {loadTestEnabled
                ? "Load-test mode — no Google sign-in. Use Test sign in to open the dashboard."
                : "Access restricted to authorised admins only."}
            </p>
          </CardContent>
        </Card>
      </div>
      <SiteCreditFooter tone="dark" />
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}
